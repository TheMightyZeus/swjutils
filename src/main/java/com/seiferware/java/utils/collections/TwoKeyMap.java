...
/**
 * An interface similar to {@link Map} that associates each value with two keys.
 * A value stored with keys {@code k1} and {@code k2} can only be retrieved with
...
public interface TwoKeyMap<K1, K2, V> {
	/**
	 * Returns the number of key-key-value mappings in this map. If the map
	 ...
	 * keys. More formally, returns <tt>true</tt> if and only if this map
	 * contains a mapping for keys <tt>k1</tt> and <tt>k2</tt> such that
	 * <tt>(key1==null ? k1==null : key1.equals(k1)) && (key2==null ? k2==null : key2.equals(k2))</tt>
	...
	/**
	 * Returns <tt>true</tt> if this map maps one or more key pairs to the
	 ...
	 * @return <tt>true</tt> if this map maps one or more key pairs to the
	 *         specified value
	 */
	public boolean containsValue(V value);
	/**
	 * Returns the value to which the specified keys are mapped, or {@code null}
	 ...
	 * keys; it's also possible that the map explicitly maps the keys to
	 * {@code null}. The {@link #containsKeys containsKey} operation may be used
	 ...
	 * @return the value to which the specified keys are mapped, or {@code null}
	 ...
	 * keys, the old value is replaced by the specified value. (A map <tt>m</tt>
	 * is said to contain a mapping for keys <tt>k1</tt>, <tt>k2</tt> if and
	 * only if {@link #containsKeys(Object, Object) m.containsKeys(k1, k2)}
	 ...
	 * @return the previous value associated with <tt>key1</tt>, <tt>key2</tt>,
	 *         or <tt>null</tt> if there was no such mapping. (A <tt>null</tt>
	 *         return can also indicate that the map previously associated
	 *         <tt>null</tt> with the keys, if the implementation supports
	 *         <tt>null</tt> values.)
	 */
	 ...
	 * (optional operation).
	 * <p>
	 * Returns the value to which this map previously associated the keys, or
	 * <tt>null</tt> if the map contained no mapping for the keys.
	 * </p>
	 * <p>
	 ...
	 * for the keys; it's also possible that the map explicitly mapped the keys
	 * to <tt>null</tt>.
	 * </p>
	 * <p>
	 * The map will not contain a mapping for the specified key pair once the
	 * call returns.
	 * </p>
	 *
	 * @param key1
	 ...
	 * @return the previous value associated with the keys, or <tt>null</tt> if
	 *         there was no such mapping.
	 */
